...
class Talker : public rclcpp::Node
{
    ...
    data_("Hello world!")
    {
        auto publish_message=
            [this]()->void
            {
                auto msg=std::make_unique<std::msgs::msg::String>();
                msg->data=data_;
        ...
        std::shared_ptr<SetMessage::Response> response)->void 
        {
            ...
            RCLCPP_INFO(this->logger(),"message %s -> %s",
            this->data_=request->message.c_str(),request->message.c._str());
            //1秒スリープ (重い処理の代わり)
...
{
    setvbuf(stdout,NULL,_IOBF.BUFSIZ);
    rclcpp::init(argc,argv);

    autonode=std::make_shared<Talker>("chatter");
    rclcpp::spin(autonode);
    rclcpp::shutdown();
    return 0;
}